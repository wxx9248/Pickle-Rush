...
class GameLost(Scene):
    def __init__(self, size: typing.Tuple[int, int], retry_scene: typing.Optional[typing.Type[Scene]]):
        ...
        game_lost_text_sprite: Text = AssetObjectFactory().new_asset_object("asset.text.game-lost")
        ...
        game_lost_text_atlas.position_y = 150

        retry_text_sprite: Text = AssetObjectFactory().new_asset_object("asset.text.retry")
        retry_text_atlas = Atlas(retry_text_sprite)
        retry_text_atlas.position_x = util.center(self.size, retry_text_sprite.surface.get_size())[0]
        retry_text_atlas.position_y = 450

        back_to_menu_text_sprite: Text = AssetObjectFactory().new_asset_object("asset.text.back-to-menu")
        back_to_menu_text_atlas = Atlas(back_to_menu_text_sprite)
        back_to_menu_text_atlas.position_x = util.center(self.size, back_to_menu_text_sprite.surface.get_size())[0]
        ...
        cursor_sprite_atlas.add_anchor(
            ...
            (back_to_menu_text_atlas.position_y - 6, lambda: self.back_to_menu())
        )
